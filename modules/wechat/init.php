...
/*
// 在任意网站地址后面加上参数 ?wipe 即可清理，比如 http://www.example.com/?wipe
if (isset($_GET['wipe'])) {
	...
	foreach ($users as $user) {
		$unionid = get_user_meta($user->ID, $meta_key, true);
		$others = get_users( array(
			'meta_key'     => $meta_key,
			'meta_value'   => $unionid,
			'meta_compare' => '='
		));
		if (!empty($others) && count($others) > 1) {
			array_shift($others);
			foreach ($others as $other) wp_delete_user($other->ID);
		}
	}
}
*/

if (!defined('WEIXINAPI')) include_once( QQWORLD_PASSPORT_DIR . 'lib' . DIRECTORY_SEPARATOR . 'wechat' . DIRECTORY_SEPARATOR . 'weixin.sdk.phar' );
...
class wechat extends core {
	...
	public function openids($content, $id) {
		$content .= '<p><strong>Wechat OpenID:</strong> <em>'. get_the_author_meta( 'QQWorld Passport Wechat Openid', $id ) . '</em><br /><strong>Wechat UnionID:</strong> <em>' . get_the_author_meta( 'QQWorld Passport Wechat Unionid', $id ) . '</em></p>';
		return $content;
	}
	...
	**/
	public function parse_request() {
		...
		if ( !isset($respond['errcode']) ) {
			...
			if (empty($openid) && empty($unionid)) return;
			...
			if (!isset($respond['errcode'])) {
				...
				if (get_option( 'users_can_register' )) {
					...
					if (isset($user_id) && $user_id) {
						...
						$this->delete_user_meta($user_id, 'QQWorld Passport Wechat Openid', $openid);
						update_user_meta( $user_id, 'QQWorld Passport Wechat Openid', $openid );
						if ($unionid) {
							// 清除所有其他可能包含'QQWorld Passport QQ Openid'的用户的这个meta
							$this->delete_user_meta($user_id, 'QQWorld Passport Wechat Unionid', $unionid);
							update_user_meta( $user_id, 'QQWorld Passport Wechat Unionid', $unionid );
						}
				...
				<tbody>
					<tr valign="top">
						...
						<td class="forminp">
							...
							<p><?php printf(__('QQWorld Synchronizer has been removed, please use <a href="%s" target="_blank">QQWorld Operating Officer</a>.', $this->text_domain), 'https://www.qqworld.org/product/qqworld-operating-officer'); ?></p>
						</td>
					</tr>
					...
					<tr valign="top">
						...
						<td class="forminp">
							...
							<p class="description"><?php printf(__("Please <a href=\"%s\" target=\"_blank\">click here</a> to create a website APP.<br />And you need install the Wechat Extends of <a href=\"%s\" target=\"_blank\">QQWorld Synchronizer</a> to use native login on PC.", $this->text_domain), 'https://open.weixin.qq.com/', 'https://www.qqworld.org/product/qqworld-synchronizer/'); ?><br /><?php printf(__('QQWorld Synchronizer has been removed, please use <a href="%s" target="_blank">QQWorld Operating Officer</a>, you can use this feature for free.', $this->text_domain), 'https://www.qqworld.org/product/qqworld-operating-officer'); ?><br /><br />
					...
					<tr valign="top">
						...
						<td class="forminp">
							...
							<p class="description"><?php printf(__('QQWorld Synchronizer has been removed, please use <a href="%s" target="_blank">QQWorld Operating Officer</a>, you can use this feature for free.', $this->text_domain), 'https://www.qqworld.org/product/qqworld-operating-officer'); ?></p>
						</td>
					</tr>
