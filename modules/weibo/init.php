...
class weibo extends core {
	...
	public function openids($content, $id) {
		$content .= '<p><strong>Weibo OpenID:</strong> <em>'. get_the_author_meta( 'QQWorld Passport Weibo Uid', $id ) . '</em></p>';
		return $content;
	}
	...
	*/
	...
	public function parse_request() {
		...
		$params = array(
			...
			'code' => $code,
			'client_id' => $this->options->moudule_weibo_appkey,
			'client_secret' => $this->options->moudule_weibo_appsecret,
			'redirect_uri' => $redirect_url
		);
		$request = "https://api.weibo.com/oauth2/access_token";

		$response = $this->curl($request, 'post', $params);
		if (isset($response->error)) {
			die($response->error . ': ' . $response->error_description);
		}
...
?>
		<a class="weibo loginbtn" href="<?php echo $dialog_url; ?>" title="<?php _e('Weibo Login', $this->text_domain); ?>"><img src="<?php echo QQWORLD_PASSPORT_URL; ?>images/icons/weibo.png" width="32" height="32" /></a>
<?php
	}
